...
さらに、任意のコメントテンプレートを使用して Issue に自動コメントを投稿できます
（投稿の有無はイベントごとに ON/OFF が可能）。
...
freee の番号（例：`#1234`）が Redmine の Issue ID と一致する場合にのみ同期します。

```
"#1234" → Issue ID 1234 を更新
```

ステータスの更新は設定画面で各イベントごとに個別に制御できます。
...
- 「変更しない」または任意のステータス
- コメントを投稿する（チェックした場合のみテンプレート入力欄が表示）
...
- freee の番号は Issue ID と一致させてください（例：`#1234`）
- 「変更しない」を選んだイベントはスキップされます
- 入金待ち (unsettled) → **未入金イベント**
- 権限エラー（401）は安全にスキップ
